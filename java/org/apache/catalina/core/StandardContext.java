...
// 使用 TreeMap 对 Wrapper 进行排序，loadOnStartup 值越小越靠前，值相同的 Wrapper 放在同一个 List 中
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        /**
         * 在 tomcat 中，每个 webapp 对应一个 StandardContext，在 start 过程便会实例化 WebappLoader，
         * 并且调用其 start 方法完成初始化，包括创建 ParallelWebappClassLoader 实例，然后，还会启动 Context 的子容器。
         * 注意，这两个过程，都会将线程上下文类加载器指定为 ParallelWebappClassLoader 类加载器，在完成 webapp 相关的类加载之后，
         * 又将线程上下文类加载器设置为 catalina 类加载器。
         * ————————————————
        ...
        if (getLoader() == null) {
            //ParallelWebappClassLoader：负责加载tomcat中每个应用的类包，每个应用对应一个 ParallelWebappClassLoader，
            // 这样子可以避免不同应用同名类加载冲突的问题，
        ...
        // Binding thread
        // 第一次：将 Loader 中的 ParallelWebappClassLoader 绑定到当前线程中，并返回 catalian 类加载器
