 ...
 * 并且预留了initInternal、startInternal、stopInternal等模板方法，便于子类完成自己的逻辑
 *
 *
 * MBeanRegistration：JmxEnabled 的父类， jmx框架提供的注册MBean的接口，引入此接口是为了便于使用JMX提供的管理功能
 *
 * LifecycleMBeanBase：JmxEnabled的子类，通过重写initInternal、destroyInternal方法，统一向jmx中注册/取消注册当前实例，
 * 方便利用jmx对实例对象进行管理，代码上特别强调要求子类先行调用super.initInternal
 *
 * ContainerBase、StandardServer、StandardService、WebappLoader、Connector、StandardContext、StandardEngine、
 * StandardHost、StandardWrapper等容器都继承了LifecycleMBeanBase，因此这些容器都具有了同样的生命周期并可以通过JMX进行管理
 */
